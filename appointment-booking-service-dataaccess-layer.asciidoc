...
[WARNING]
====
Explicit *spring.jpa.open-in-view=false* is the best practice in most cases. More details: how to overcome
https://stackoverflow.com/questions/30549489/what-is-this-spring-jpa-open-in-view-true-property-in-spring-boot[The OSIV Anti-Pattern ]
====
...
**Why do we need the Maven plugin?**

The `apt-maven-plugin` tells Maven:

* **When to run**: During the `generate-sources` phase (before compilation)
* **What processor to use**: `JPAAnnotationProcessor` to scan JPA entities
* **Where to put generated code**: `target/generated-sources/java` directory
...
* ❌ **Missing plugin**: Q-classes not generated during Maven build

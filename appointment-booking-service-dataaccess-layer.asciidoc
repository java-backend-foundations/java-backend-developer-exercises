...
Create a new test class named _EntitySmokeTest_ in _src/main/test_, under _com.capgemini.training.appointmentbooking.dataaccess.entity_. Annotate it with `@DataJpaTest(bootstrapMode = BootstrapMode.LAZY)` to configure JPA-related components.
...
Then we will create a _BaseDataJpaTest_ that enables

* Persistance integration testing with _@DataJpaTest_ annotation
* Support for transaction (and automatic rollback) _@Transactional_ annotation
* Enable usage of our custom repository implementation via _@Import_ annotation

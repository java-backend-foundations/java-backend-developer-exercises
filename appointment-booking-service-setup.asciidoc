...
[NOTE]
====
Spring Boot's parent POM manages plugin versions automatically, so explicit version declarations are unnecessary. This approach ensures compatibility and reduces maintenance overhead.
====
